...
import torch
from scene.gaussian_model_ori import build_scaling_rotation
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, loss_type,coarse_iteration, graph_sample,elaboration_iterations):
    ...
    gaussians = GaussianModel(dataset.sh_degree, dataset.smpl_type, dataset.motion_offset_flag, dataset.actor_gender, pipe.add)
    ...
    gaussians._objects_dc = F.one_hot(gaussians.frozen_labels.to(torch.int64),num_classes=16).unsqueeze(1).to(torch.float32)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if (iteration - 1) == debug_from:
            pipe.debug = True
        # gaussians._objects_dc = F.one_hot(gaussians.frozen_labels.to(torch.int64), num_classes=15).unsqueeze(1).to(torch.float32)
        ...
        if iteration % 100 == 0 :
            libcore.write_tensor_image(os.path.join('output/train', f'render_{iteration}.jpg'), image, rgb2bgr=True)
        ...
        if iteration > coarse_iteration:    
            ...
            for i in types_mapping.keys():
                ...
                img_clip_loss = img2img_clip_loss(clip_model, (image * object_mask).unsqueeze(0),(gt_image * object_mask).unsqueeze(0))
                text_clip_loss = img2text_clip_loss(clip_model, "a image of " + types_mapping[i],(image * object_mask).unsqueeze(0))
                clip_loss = img_clip_loss + text_clip_loss if clip_loss is None else clip_loss + img_clip_loss + text_clip_loss
                # clip_loss = clip_loss / 16
            loss += clip_loss
        ...
        if iteration > elaboration_iterations:
            ...
            # data = nodeEmbedding_node2vec(graph).cuda()    # N,64
            # data = nodeEmbedding_node2vec_new(graph).cuda() 
            data = nodeEmbedding_gat(gaussians, gaussians._xyz.detach().cpu().numpy(), features.detach().cpu().numpy(), 3)
            clu_loss = None
            ...
            loss_GAT = clu_loss / (graph_sample * 10)
            # loss_GAT.backward()
            loss += loss_GAT
        ...
        with torch.no_grad():
            ...
            if iteration < opt.iterations:
                ...
                if iteration > elaboration_iterations:
                    gaussians.optimizer_GAT.step()
                    gaussians.optimizer_GAT.zero_grad(set_to_none=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--elaboration_iterations", type=int, default=2000)
