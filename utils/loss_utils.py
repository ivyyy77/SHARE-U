...
def fourier(img_part, gt_part, object_mask):
    restored_img_render = torch.zeros(512,512,3).to('cuda')
    restored_img_gt = restored_img_render
    restored_img_render[object_mask[0] == 1] = img_part
    img_np_render = restored_img_render.detach().cpu().numpy()
    img_np_render = cv2.cvtColor(img_np_render, cv2.COLOR_RGB2GRAY)
    laplacian_render = cv2.Laplacian(img_np_render, cv2.CV_32F)
    high_freq_render = np.abs(laplacian_render)
    img_part = torch.tensor(high_freq_render)

    restored_img_gt[object_mask[0] == 1] = gt_part
    imp_np_gt = restored_img_gt.detach().cpu().numpy()
    imp_np_gt = cv2.cvtColor(imp_np_gt, cv2.COLOR_RGB2GRAY)
    laplacian_gt = cv2.Laplacian(imp_np_gt, cv2.CV_32F)
    high_freq_gt = np.abs(laplacian_gt)
    gt_part = torch.tensor(high_freq_gt)

    img_part = torch.fft.fft2(img_part)
    gt_part = torch.fft.fft2(gt_part)
    fourier_loss = F.l1_loss(img_part, gt_part)
    return fourier_loss
...
def img2text_clip_loss(clip_model, text, image):
...
def img2img_clip_loss(clip_model, image, image2):
    # clip_model, clip_preprocess = clip.load("assets/ViT-B-16.pt", device='cuda', jit=False)
    image = F.interpolate(image, size=(224, 224), mode='bilinear', align_corners=False)
