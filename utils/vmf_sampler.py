...
class VonMisesFisher(torch.distributions.Distribution):
  ...
  def rsample(self, shape=torch.Size()):
    shape = shape if isinstance(shape, torch.Size) else torch.Size([shape])   # 512
    ...
    w = (self.__sample_w3(shape=shape) if self.__m == 3
         else self.__sample_w_rej(shape=shape))     # 512,1000,1,1
